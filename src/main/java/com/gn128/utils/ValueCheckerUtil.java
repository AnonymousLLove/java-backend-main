...
public final class ValueCheckerUtil {
    ...
    public static void isValidUUID(String uuid) {
        try {
            ...
            throw new BadRequestException("Invalid UUID", HttpStatus.BAD_REQUEST);
        }
    }

    public static void isValidUUID(String uuid, String exceptionMessage) {
        try {
            UUID response = UUID.fromString(uuid);
        } catch (IllegalArgumentException ignored) {
            throw new BadRequestException(exceptionMessage, HttpStatus.BAD_REQUEST);
        }
    }
}
