...
import com.gn128.repository.RegistrationOtpRepository;
import com.gn128.utils.OtpGenerator;
...
public class SendRegistrationOtpProcessor {

    private static final Logger logger = LoggerFactory.getLogger(SendRegistrationOtpProcessor.class);

    private final OtpGenerator otpGenerator;
    private final RegistrationOtpRepository registrationOtpRepository;
    private final RegistrationOtpMailSendingProcessor registrationOtpMailSendingProcessor;

    @SneakyThrows
    public void process(UserAuth userAuth) {
        if (Boolean.TRUE.equals(userAuth.isEnabled())) {
            logger.error("User is already enabled or email is already verified");
        } else {
    ...
    @SneakyThrows
    public void process(UserAuth userAuth, int timesSend) {
        if (Boolean.TRUE.equals(userAuth.isEnabled())) {
            logger.error("User is already enabled or email is already verified");
        } else {
            RegistrationOtp registrationOtp = otpGenerator.registrationOtpSupplier(userAuth);
            registrationOtp.setTimesSent(timesSend);
            registrationOtpMailSendingProcessor.sendMail(registrationOtp);
            registrationOtpRepository.save(registrationOtp);
        }
    }
